...
async signup(data: SignupDto) {
  ...
  const user = this.userRepository.create({ email: normalizedEmail, password: hashedPassword });
  await this.userRepository.save(user);

  const name = normalizedEmail.split('@')[0];
  this.emailClient.emit('user_signup', { email: normalizedEmail, name });
...
async login(email: string, password: string) {
  ...
  this.emailClient.emit('user_login', { email: user.email, time: new Date().toISOString() });
